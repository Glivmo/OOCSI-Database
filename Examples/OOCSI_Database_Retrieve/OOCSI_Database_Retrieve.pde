...
void draw() {
  ...
  if (mousePressed) {
    ...
    oocsi.channel("Tyria_World_Data_Get")
      ...
      .data("timeStamp", 2270700 + (i-1))  //Requested timeStamp
...
void handleOOCSIEvent(OOCSIEvent event) {
  ...
  if (sender.equals("Tyria_World_Database")) {
    ...
    if (!event.getString("debug", "").equals(""))println(event.getString("debug", ""));                         //if timeStamp is not available in database, the closest timeStamp will be used 
